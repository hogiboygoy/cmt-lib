...
CmtSpectrometr::CmtSpectrometr(int pow) :
  T(pow) {
    //Исходный массив
    mArr = new ShortComplex[Count()];
    ...
    mAmp = new double[Nmax];

    //Промежуточный массив
    Wstore = new ShortComplex[Nmax];
    }

CmtSpectrometr::~CmtSpectrometr() {
  if( mArr ) delete mArr;
  if( mAmp ) delete mAmp;
...
*/
void 
CmtSpectrometr::Fft( bool complement ) {
